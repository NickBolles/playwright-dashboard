...
export function RecentRuns({ runs }: RecentRunsProps) {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <Badge 
        variant="outline" 
        className={cn('border', variants[status as keyof typeof variants] || variants.cancelled)}
      >
        {status.replace('_', ' ')}
      </Badge>
    )
  ...
  if (!runs.length) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-sm text-muted-foreground">Start your first test run to see results here</p>
      </div>
    )
  }
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <div className="text-sm text-muted-foreground">
                {run.environment.name} • {formatDistanceToNow(new Date(run.created_at), { addSuffix: true })}
              </div>
            </div>
          </div>
